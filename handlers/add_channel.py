...
async def add_channel(message: types.CallbackQuery,state:FSMContext) -> None:
    ...
    await message.message.answer(ADD_CHANNEL, reply_markup=add_channel_request(), disable_web_page_preview=True)
...
async def channel_id_get(message: types.Message,state: FSMContext):
    try:
        ...
        if len(find)==0:
            ...
            await message.answer(CHANNEL_SAVED, reply_markup=ReplyKeyboardRemove(), disable_web_page_preview=True)
            await message.answer(WHT_IS_GRT_MSG, reply_markup=defaultn(), disable_web_page_preview=True)
        else:
            await message.answer(CHNL_ALRDY_ADDED, reply_markup=add_channel_request(), disable_web_page_preview=True)
...
async def channel_greet_get(callback: types.CallbackQuery,state:FSMContext):
    ...
    if callback.data == 'default':
        ...
        await callback.message.answer(CHANNEL_INSERTED, reply_markup=ReplyKeyboardRemove(), disable_web_page_preview=True)
        ...
        await callback.message.delete()
        await callback.message.answer(SEND_GREETING_MESSAGE, reply_markup=get_n_cancel(), disable_web_page_preview=True)
...
async def channel_btn_get(callback: types.CallbackQuery,state:FSMContext):
    ...
    if callback.data == 'Yes':
        ...
        await callback.message.reply(CANCELLED, reply_markup=get_keyboard(), disable_web_page_preview=True)
        await callback.message.delete()
        await state.clear()
    else:
