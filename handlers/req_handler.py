            ...
            if details[0]['greet_msg']!=0:
                ...
                if details[0]['btns'] !=0 and details[0]['btns']  not in ['None','0']:
                    try:
                        ...
                        try:
                            # Sends the stored snapshot when there is one, so any
                            # premium / animated emoji the owner used survive;
                            # falls back internally to copying their original.
                            await send_stored_message(
                                message.bot, message.from_user.id, details[0].get('greet_data'),
                                details[0]['greet_msg_chat'], details[0]['greet_msg'],
                                reply_markup=None if details[0]['btns'] == 'None'
                                else InlineKeyboardBuilder(buttons).as_markup())
                        except Exception as copy_err:
                            print(f"[DEBUG join_request] greeting send FAILED: {copy_err!r} greet_msg_chat={details[0]['greet_msg_chat']!r} greet_msg={details[0]['greet_msg']!r}")
                            raise
