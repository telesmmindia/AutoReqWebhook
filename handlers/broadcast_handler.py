...
async def add_channel(message: types.CallbackQuery,state:FSMContext) -> None:
    ...
    await message.message.edit_text(TOTAL_USERS_MESSAGE.format(clients["count(*)"]),reply_markup=my_users_btn(), disable_web_page_preview=True)
...
async def schedule_handle(message: types.CallbackQuery,state: FSMContext):
    if message.data == 'all_user':
        ...
        await message.message.answer(BROADCAST_MESSAGE_PROMPT,reply_markup=get_n_cancel(), disable_web_page_preview=True)

    elif message.data == 'channel_wise':
        await state.set_state(my_users.channels_run)
        await state.update_data(users='channel')
        await message.message.edit_text(SELECT_CHANNEL_PROMPT,reply_markup=channels_new(get_channels(message.from_user.id)), disable_web_page_preview=True)

    elif message.data == 'cancel':
        await message.message.edit_text(CHOOSE, reply_markup=get_keyboard(), disable_web_page_preview=True)
        await state.clear()
...
async def schedule_handle(callback: types.CallbackQuery,state: FSMContext):
    if callback.data == 'cancel':
        await state.set_state(my_users.channels)
        await callback.message.edit_text(CHOOSE, reply_markup=my_users_btn(), disable_web_page_preview=True)
    else:
...
async def schedule_handle(message: types.Message, state: FSMContext):
    ...
    if "cancel" in message.text.lower():
        ...
        await message.answer(TOTAL_USERS_MESSAGE.format(clients["count(*)"]), reply_markup=my_users_btn(), disable_web_page_preview=True)
        await state.set_state(my_users.channels)
    elif data['users'] == 'all':
        if message.text.isnumeric():
            await state.update_data(users_count = message.text)
            await state.set_state(my_users.channels_run_send_con)
            await message.answer(SEND_MESSAGE_PROMPT,reply_markup=get_n_cancel(), disable_web_page_preview=True)
        else:
            await message.answer(ENTER_NUMBER_ONLY, disable_web_page_preview=True)
    else:
        await state.update_data(channel=message.text)
        await state.set_state(my_users.channels_run)
        await message.answer(SEND_TO_USERS_PROMPT, disable_web_page_preview=True)
...
async def schedule_handle(callback: types.CallbackQuery,state: FSMContext):
    if callback.data == 'back':
        ...
        await callback.message.answer(USER_COUNT_BROADCAST.format(all_clients(callback.from_user.id, callback.data)[0]["count(*)"]),reply_markup=get_n_cancel(), disable_web_page_preview=True)
...
async def schedule_handle(message: types.Message,state: FSMContext):
    if message.text.isnumeric():
        await state.update_data(users_count=message.text)
        await state.set_state(my_users.channels_run_send_con)
        await message.answer(FORWARD_YOUR_POST, disable_web_page_preview=True)

    elif "cancel" in message.text.lower():
        await state.set_state(my_users.channels)
        await message.reply(CANCELLED, reply_markup=ReplyKeyboardRemove(), disable_web_page_preview=True)
        await message.answer(CHOOSE,reply_markup=my_users_btn(), disable_web_page_preview=True)
    else:
        await message.answer(ENTER_NUMBER_ONLY, disable_web_page_preview=True)
...
async def edit_message(callback: types.CallbackQuery,state: FSMContext):
    ...
    if callback.data == 'Yes':
        ...
        if data['users'] != 'all':
            ...
            if len(clients) == 0:
                await callback.message.edit_text(NOT_ENOUGH_PEOPLE, reply_markup=get_keyboard(), disable_web_page_preview=True)
            else:
                await callback.message.answer(SENDING_MESSAGE_TO_USERS, reply_markup=ReplyKeyboardRemove(), disable_web_page_preview=True)
                await callback.message.answer(CHOOSE, reply_markup=get_keyboard(), disable_web_page_preview=True)
